...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Mise à jour en base
    $sql = "UPDATE etudiants SET nom = ?, prenom = ?, profession = ?, bio = ?, photo_profil = ?, ville = ?, quartier = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssssi", $nom, $prenom, $profession, $bio, $photo_profil, $ville, $quartier, $user_id);
...
$sql = "SELECT nom, prenom, profession, bio, photo_profil, ville, quartier FROM etudiants WHERE id = ?";
$stmt = $conn->prepare($sql);
...
<form method="post" enctype="multipart/form-data">
    ...
    <input type="text" name="profession" value="<?= htmlspecialchars($user['profession']) ?>" required><br><br>
 ...
 <label>Ville :</label><br>
    <input type="text" name="ville" value="<?= htmlspecialchars($user['ville']) ?>" required><br><br>

    <label>Quartier :</label><br>
    <input type="text" name="quartier" value="<?= htmlspecialchars($user['quartier']) ?>" required><br><br>

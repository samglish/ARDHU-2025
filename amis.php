...
echo "<h3 class='mt-4'>Se connecter aux membres</h3> <div id='Formulaire'>";
echo "<form method='POST' class='add-friend-form'>
    <div class='input-group mb-3'>
        <input type='text' class='form-control' placeholder='Entrez un email' name='email' required>
    ...
    if (!empty($email)) {
        ...
        if ($target) {
            ...
            if ($check->get_result()->num_rows == 0) {
                ...
                $insert->execute();
                echo "<div class='alert alert-success'>Demande envoyée à $email</div>";
            } else {
                echo "<div class='alert alert-warning'>Relation déjà existante</div>";
            }
        } else {
            echo "<div class='alert alert-danger'>Aucun étudiant trouvé avec cet email</div>";
        }
    }
}
...
$pending = $conn->query("
    SELECT e.id, e.nom, e.prenom, e.email, a.id as request_id 
    FROM amis a 
    ...
    while ($req = $pending->fetch_assoc()) {
        ...
        <div class='request-card'>
            <div class='request-info'>
                <strong>{$req['prenom']} {$req['nom']}</strong> ({$req['email']})
            </div>
...
$sql = "
    SELECT e.id, e.nom, e.prenom, e.photo_profil, e.email
    FROM amis a
    JOIN etudiants e ON 
...
$sql_all = "
    SELECT e.id, e.nom, e.prenom, e.email, e.photo_profil
    FROM etudiants e
    WHERE e.id != ?
    AND e.id NOT IN (
